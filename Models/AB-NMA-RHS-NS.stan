...
// cholesky decomp: Sigma = OGG'O
// RHS prior on elements of O
// weak priors on elements of G
...
transformed parameters {
  ...
  vector[n_gamma] gamma; // scaled gamma vector
...
model {
  ...
  for(j in 1:nstudy){
    vi[j,]~ multi_normal_cholesky(zeros, Sigma_chol);
  }
    for(i in 1:len){
